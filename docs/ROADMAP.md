...
- API backend apenas para read-only, dry-run e diff-preview: iniciada e validada naquele checkpoint com 84/84 testes; a consolidacao final da branch elevou o backend para 123/123.
- Apply/rollback via API deve ficar bloqueado por feature flag/confirmacao forte ate nova decisao.
- API hardening com auth local, ProblemDetails, operation guards, rate/concurrency, CORS restrito e correlationId: concluido e consolidado com 123/123 testes no backend.
...
- Totais consolidados desta branch:
  - Backend: 123/123 testes
  - Frontend: 30/30 testes
  - Agent: 179/179 testes
