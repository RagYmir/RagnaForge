...
- API backend apenas para read-only, dry-run e diff-preview: iniciada e validada naquele checkpoint com 84/84 testes; a consolidacao final da branch elevou o backend para 126/126.
- Apply/rollback via API deve ficar bloqueado por feature flag/confirmacao forte ate nova decisao.
- API hardening com auth local, ProblemDetails, operation guards, rate/concurrency, CORS restrito e correlationId: concluido e consolidado com 126/126 testes no backend.
